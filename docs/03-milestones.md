...
当前入口例外（2026-09-07）：依据[维护者决策](decisions/single-agent-m0.md)，单 Agent 工作允许从 G0 进入 M0，无需等待 G1 演练。G1 的完整验收标准保留，恢复并发 Agent 写入前必须补齐。后续产品、安全和发布门槛不变。

```mermaid
flowchart LR
    G0["G0 仓库与发布基础设施"] --> G1["G1 多 Agent 协作基线"]
    G1 --> M0["M0 契约与安全基线"]
    G0 -. "维护者批准：单 Agent 路径" .-> M0
    M0 --> C1["C1 环境发现与报告契约"]
    C1 --> C2["C2 兼容规则引擎"]
    C2 --> C3["C3 候选版本分析"]
    C2 --> C4["C4 隔离启动验证"]
...
当前状态：多 Agent 演练延期，未验收；不阻塞已批准的单 Agent M0 路径。以下条件仍是未来启用多 Agent 模式的验收标准。
...
- G1 完成，或满足[维护者批准的单 Agent 入口例外](decisions/single-agent-m0.md)；
- 本目录的设计文档进入评审；
