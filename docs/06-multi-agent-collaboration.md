...
当前模式（2026-09-07）：维护者因预算限制决定延期多 Agent 演练，按[单 Agent 入口例外](decisions/single-agent-m0.md)进入 M0。本文的完整 G1 验收条件在恢复多 Agent 模式时执行；当前不主动启动演练或委派，也不将延期记为验收通过。单 Agent 仍遵守任务范围、认领、分支、PR 和 CI 规则。
